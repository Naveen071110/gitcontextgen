...
export async function executeAnalyze(targetPath, options = {}) {
    ...
    if (analysis.wordpress?.isWordPress) {
        ...
        if (analysis.wordpress.version)
            console.log(`  ↳ Version: ${analysis.wordpress.version}`);
        if (analysis.wordpress.hasBlockJson)
            console.log(`  ↳ Gutenberg Block Support (block.json)`);
        if (analysis.wordpress.hasTelex)
            console.log(`  ↳ Automattic Telex Compatible`);
        if (analysis.wordpress.hasWpCli)
            console.log(`  ↳ wp-cli Configuration Detected`);
    }
