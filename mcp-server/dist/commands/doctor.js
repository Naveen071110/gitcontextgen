 ...
 */
export function auditInstalledIDEs(workspaceDir) {
    const isWindows = process.platform === 'win32';
 ...
 */
export function registerClaudeDesktopMcp() {
    try {
 ...
 */
export function safelyEstablishSymlink(sourcePath, linkPath) {
    try {
...
export async function executeDoctor(targetPath, options = {}) {
    ...
    try {
        ...
        let score = 70;
        if (mcpRegistered.length >= 2)
            score += 15;
        else if (mcpRegistered.length === 1)
            score += 10;
        if (hasClaude || hasAgents)
            score += 10;
        if (fs.existsSync(path.join(workspaceDir, '.cursor', 'rules')))
            score += 5;
        console.log('\n' + '='.repeat(72));
