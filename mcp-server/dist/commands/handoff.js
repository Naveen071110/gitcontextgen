...
export function translateCommitToBusinessValue(subject) {
    ...
    if (/billing|dodo|stripe|payment|checkout|subscription|customer-portal/i.test(s)) {
        return {
            ...
            businessValue: 'Deployed secure Merchant-of-Record subscription management with self-service checkout sessions, automatic invoice generation, and customer portal access.',
            isNoise,
        };
    }
    if (/wordpress|wpcs|wp-cli|telex|gutenberg|block\.json/i.test(s)) {
        return {
            ...
            businessValue: 'Enforced official WordPress Coding Standards (WPCS), input sanitization, and automated database preparation gates for high-performance plugin and theme safety.',
            isNoise,
        };
    }
    if (/filelock|lock|concurrency|multi-agent|race condition|conflict/i.test(s)) {
        return {
            ...
            businessValue: 'Integrated atomic cross-platform file locking to eliminate file collisions, preventing overlapping edits and race conditions across distributed developers.',
            isNoise,
        };
    }
    if (/doctor|onboard|init|setup|wizard/i.test(s)) {
        return {
            ...
            businessValue: 'Built automated environment diagnostic wizards that detect host IDE configurations (Claude, Cursor, Windsurf) and automatically connect tooling without manual friction.',
            isNoise,
        };
    }
    if (/lint|rules|cursor|\.mdc|claude\.md|agents\.md|sync/i.test(s)) {
        return {
            ...
            businessValue: 'Synchronized universal AI coding guidelines across team repositories, eliminating AI context drift and preventing model hallucinations.',
            isNoise,
        };
    }
    if (/security|vulnerability|audit|leak|xss|sanitize|osv/i.test(s)) {
        return {
            ...
            businessValue: 'Hardened application security boundaries with automated credential leak detection and strict input sanitization shields.',
            isNoise,
        };
    }
    if (/mcp|cache|l2|storage|speed|latency/i.test(s)) {
        return {
            ...
            businessValue: 'Activated high-speed disk caching to slash tool response latencies to 1ms, reducing recurring AI token burn by up to 92%.',
            isNoise,
        };
    }
    if (/dashboard|ui|frontend|page|component|tailwind/i.test(s)) {
        return {
            ...
            businessValue: 'Delivered responsive management interfaces with zero-delay mobile interactions, interactive analytics visualizers, and team seat tracking.',
            isNoise,
        };
    }
 ...
 */
export function extractGitCommits(repoDir, sinceNormalized) {
    try {
        const output = execSync(`git log --since="${sinceNormalized}" --pretty=format:"%H|%an|%ad|%s" --date=short --no-merges`, {
            cwd: repoDir,
            encoding: 'utf-8',
        ...
        for (const line of lines) {
            ...
            if (parts.length >= 4) {
                ...
                const { category, businessValue, isNoise } = translateCommitToBusinessValue(subject);
                commits.push({
                    hash,
        ...
        console.warn(`[Handoff Warning] Could not read git history: ${err.message}`);
        return [];
    }
}
/**
 ...
 */
export function buildHandoffReportData(repoDir, options) {
    ...
    const capabilities = Array.from(categoryMap.entries()).map(([category, itemsSet]) => ({
        category,
        items: Array.from(itemsSet),
    }));
    const featureCommitsCount = commits.filter((c) => !c.isNoise).length;
    const executiveSummary = `During this billing cycle, our engineering team focused on accelerating system velocity, ` +
 ...
 */
export async function executeHandoff(targetPath, options = {}) {
    ...
    console.log(`   👉 ${outPath}\n`);
    if (format === 'pdf') {
        console.log('ℹ️  Note on PDF Format: A print-optimized HTML document has been generated at the destination.');
        console.log('   Open the file in Chrome or Safari and select "Print > Save as PDF" for pixel-perfect client delivery.\n');
    }
