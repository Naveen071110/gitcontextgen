...
export async function executeInit(options = {}) {
    ...
    if (isWordPress) {
        ...
        if (wpDetection.hasBlockJson)
            console.log(`     ↳ Gutenberg block.json schema active`);
        if (wpDetection.hasTelex)
            console.log(`     ↳ Automattic Telex compatibility enabled`);
        if (wpDetection.hasWpCli)
