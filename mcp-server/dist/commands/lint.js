...
function isFileGitIgnored(filePath, repoRoot) {
    try {
 ...
 */
function scanWorkspaceForSensitiveFiles(dir, baseDir = dir) {
    const leaks = [];
    try {
 ...
 */
export async function executeLint(targetPath, options = {}) {
    ...
    if (!hasClaudeMd && !hasAgentsMd) {
        ...
        if (hasClaudeMd && mdcFiles.length > 0) {
            ...
            const mentionsRules = claudeContent.includes('.cursor') ||
