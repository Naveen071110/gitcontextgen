 ...
 */
export async function analyzeLocalDirectory(targetPath, customExcludes = []) {
    ...
    let licenseSpdx = undefined;
    const licensePath = path.join(resolvedPath, 'LICENSE');
    if (fs.existsSync(licensePath)) {
        try {
            const licText = fs.readFileSync(licensePath, 'utf-8');
            if (licText.includes('MIT License'))
                licenseSpdx = 'MIT';
            else if (licText.includes('Apache License') && licText.includes('2.0'))
                licenseSpdx = 'Apache-2.0';
            else if (licText.includes('GNU GENERAL PUBLIC LICENSE') && licText.includes('Version 3'))
                licenseSpdx = 'GPL-3.0';
            else if (licText.includes('BSD 3-Clause'))
