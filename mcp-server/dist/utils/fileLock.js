 ...
 */
export function getLocksDirectory() {
 ...
 */
export function getLockFilePath(targetFilePath) {
 ...
 */
export async function acquireFileLock(targetFilePath, options = {}) {
    const { timeoutMs = 5000, staleMs = 30000, agentId = `agent-${process.pid}-${Math.random().toString(36).slice(2, 8)}`, retryIntervalMs = 75, } = options;
    const resolved = path.resolve(targetFilePath);
    ...
    throw new Error(`[FileLock Timeout] Could not acquire exclusive lock on "${path.basename(resolved)}" within ${timeoutMs}ms. Another active agent session holds the lock.`);
}
/**
 ...
 */
export async function withFileLock(targetFilePath, operation, options = {}) {
 ...
 */
export function computeFileContentHash(filePath) {
    try {
 ...
 */
export function recordGitWorkingState(targetFilePath, cwd = process.cwd()) {
 ...
 */
export function verifyGitStateIntent(initialSnapshot, targetFilePath, cwd = process.cwd()) {
    ...
    if (initialSnapshot.fileHash !== currentHash) {
        ...
        return {
            clean: false,
            conflictReason: `Concurrent modification conflict: "${path.basename(target)}" was modified by another agent or external process after session start.`,
            currentHash,
 ...
 */
export async function safeWriteWithVerification(targetFilePath, newContent, initialSnapshot, options = {}) {
    return withFileLock(targetFilePath, async () => {
        ...
        if (!verification.clean && !options.force) {
            const warning = `[GitContextGen Merge Guard] Aborted write to "${path.basename(targetFilePath)}": ${verification.conflictReason}\nReconciliation required. Pass --force to override.`;
