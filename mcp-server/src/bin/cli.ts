  ...
  program
    ...
    .action(async () => {
      try {
        ...
        console.error('\n❌ MCP server failed:', (err as Error).message);
        process.exit(1);
      }
    });
  ...
  program
    ...
    .action(async (targetPath, options) => {
      try {
        ...
        console.error('\n❌ Rules generation failed:', (err as Error).message);
        process.exit(1);
      }
    });
  ...
  program
    ...
    .action(async (targetPath, options) => {
      try {
        ...
        console.error('\n❌ Architecture mapping failed:', (err as Error).message);
        process.exit(1);
      }
    });
  ...
  program
    ...
    .action(async (targetPath, options) => {
      try {
        ...
        console.error('\n❌ Handoff report generation failed:', (err as Error).message);
        process.exit(1);
      }
    });
